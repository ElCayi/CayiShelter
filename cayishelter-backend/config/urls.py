...
urlpatterns = [
    ...
    # Endpoint used by frontend to submit new password payload.
    path("api/auth/password/reset/confirm/", PasswordResetConfirmView.as_view(), name="password_reset_confirm_post"),
    # URL generated by dj-rest-auth in reset emails (must accept uid/token args).
    path(
        "api/auth/password/reset/confirm/<str:uid>/<str:token>/",
        lambda request, uid, token: HttpResponseRedirect(f"{settings.FRONTEND_URL}/reset-password/{uid}/{token}"),
